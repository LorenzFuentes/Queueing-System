...
public function viewFirstTable(){
  ...
  public function viewApproveTable(){
    ...
    foreach ($result as $data) {
        ...
        <tr>
          ...
          <td>
          <button class='set delete'><span class='delete'><a class='delete' href='dashboard.php?delete=$data[id]'>
             Update Que
            </a></span></button>
          </td>
        </tr>"
  ...
  public function viewCurrentTable(){
    $con = $this->con();
    ...
    foreach ($result as $data) {
      echo "
      <h1 class='text-center serv-txt'>$data[id]</h1>";
    };
  }
